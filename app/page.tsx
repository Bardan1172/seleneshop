    ...
    <div className="bg-[#02020a] min-h-screen text-white selection:bg-purple-500/30 font-sans scroll-smooth overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-50 bg-black/40 backdrop-blur-xl border-b border-white/5 px-6">
        <div className="max-w-7xl mx-auto h-20 flex items-center justify-between">
          <a href="#beranda" className={`${fantasyFont} text-2xl bg-clip-text text-transparent bg-gradient-to-r from-purple-200 to-purple-400 font-bold py-4 inline-block`}>
            SELENE SHOP 🌙
          </a>
      ...
      <main className="relative">
        ...
        {/* HERO SECTION */}
        <section id="beranda" className="relative z-10 min-h-screen flex flex-col items-center justify-center text-center px-6 pt-20">
          <h1 className={`${fantasyFont} text-7xl md:text-9xl font-bold text-white mb-10 tracking-tighter leading-tight py-6 block`}>
            SELENE <span className="text-transparent bg-clip-text bg-gradient-to-b from-purple-100 via-purple-300 to-purple-600">SHOP</span>
          </h1>
          <p className="text-white/60 max-w-2xl text-sm md:text-lg mb-14 leading-relaxed uppercase tracking-[0.3em] font-light">
            Eksplorasi estetika malam dalam dunia digital. Kami menghadirkan karya berkualitas tinggi untuk identitas Minecraft Anda.
          </p>
          
          {/* SYMMETRICAL SHORTCUT BUTTONS */}
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 w-full max-w-5xl px-4">
            {["TENTANG", "KEUNGGULAN", "LAYANAN", "CARA ORDER", "PEMBAYARAN"].map((label) => (
              <a 
                ...
                className="w-full text-center px-4 py-5 border border-white/10 hover:border-purple-500/50 hover:bg-purple-500/10 hover:shadow-[0_0_25px_rgba(168,85,247,0.15)] rounded-2xl transition-all duration-300 font-bold text-[10px] uppercase tracking-[0.2em] backdrop-blur-sm bg-white/5"
              >
                {label}
              </a>
            ))}
        ...
        <section id="tentang" className="relative z-10 py-32 px-6">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <h2 className={`${fantasyFont} text-5xl md:text-6xl font-bold text-white mb-10 leading-normal py-4`}>TENTANG KAMI</h2>
            ...
            <div className="grid grid-cols-2 gap-8 max-w-xl mx-auto">
              {[{v: "100+", l: "PROJECT SELESAI"}, {v: "PREMIUM", l: "KUALITAS KARYA"}].map((st, i) => (
                <div key={i} className="p-10 rounded-[40px] bg-white/[0.03] border border-white/10 hover:border-purple-500/50 hover:bg-white/[0.06] transition-all duration-500 group">
                  <h4 className={`${fantasyFont} text-4xl text-purple-300 font-bold mb-2 group-hover:scale-110 transition-transform`}>{st.v}</h4>
                  <p className="text-[10px] text-white/40 uppercase tracking-widest font-black">{st.l}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* SECTION 3: LAYANAN (FULL 6 CARDS) */}
        <section id="layanan" className="relative z-10 py-32 px-6">
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-24">
              ...
              <h2 className={`${fantasyFont} text-6xl md:text-7xl font-bold text-white leading-tight py-4`}>LAYANAN KREATIF</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {services.map((s, i) => (
                <div key={i} className="group relative bg-white/[0.02] border border-white/5 p-12 rounded-[50px] hover:border-purple-500/50 hover:bg-purple-900/5 hover:-translate-y-3 transition-all duration-500 flex flex-col h-full shadow-2xl">
                  <div className="absolute -top-10 -right-10 w-40 h-40 bg-purple-600/5 blur-[60px] group-hover:bg-purple-600/15 transition-all duration-700" />
                  <div className="flex-grow relative z-10">
                    <h3 className={`${fantasyFont} text-2xl font-bold text-white mb-2 leading-relaxed`}>{s.title}</h3>
                    <p className="text-[10px] text-purple-400 font-bold mb-10 tracking-[0.2em]">{s.tag}</p>
                    <div className="space-y-10 mb-12">
                      {s.items.map((item, idx) => (
                        <div key={idx} className="group/item">
                          <p className="text-[12px] font-black text-white/90 tracking-[0.15em] mb-2 group-hover/item:text-purple-300 transition-colors uppercase">{item.label}</p>
                          <p className="text-[11px] text-white/30 leading-relaxed uppercase tracking-tighter">{item.desc}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  <a href="https://discord.gg/muH44HDrea" target="_blank" className="relative z-10 block w-full text-center py-5 bg-white/5 border border-white/10 hover:bg-white hover:text-black rounded-2xl font-bold text-xs transition-all duration-500 uppercase tracking-[0.2em] mt-auto shadow-lg">
                    {s.btn}
                  </a>
                </div>
              ))}
        ...
        <section id="caraorder" className="relative z-10 py-32 px-6">
          <div className="max-w-6xl mx-auto">
            <div className="text-center mb-24">
              ...
              <h2 className={`${fantasyFont} text-5xl md:text-6xl font-bold text-white leading-normal py-4`}>CARA ORDER</h2>
            </div>
            <div className="grid md:grid-cols-3 gap-8">
              {[
                { s: "01", t: "JOIN DISCORD", d: "Masuk ke server kami melalui link undangan resmi Selene Shop." },
                { s: "02", t: "CREATE TICKET", d: "Buka tiket sesuai kategori layanan yang Anda butuhkan." },
                { s: "03", t: "PAYMENT", d: "Selesaikan pembayaran aman via QRIS atau E-Wallet pilihan Anda." }
              ].map((item, idx) => (
                <div key={idx} className="group relative p-12 rounded-[45px] bg-white/[0.02] border border-white/5 hover:border-purple-500/40 transition-all duration-500 hover:bg-white/[0.04] overflow-hidden">
                  <span className={`${fantasyFont} text-7xl font-black text-white/[0.04] absolute top-6 right-8 group-hover:text-purple-500/10 transition-all`}>{item.s}</span>
        ...
        {/* SECTION 5: PAYMENT */}
        <section id="pembayaran" className="relative z-10 py-40 px-6 bg-gradient-to-b from-transparent via-purple-500/[0.02] to-transparent">
          <div className="max-w-4xl mx-auto text-center">
            <span className="text-purple-400 text-[10px] font-bold tracking-[0.5em] mb-10 block uppercase">TRANSACTION</span>
            <h2 className={`${fantasyFont} text-4xl md:text-5xl font-bold text-white mb-16 leading-normal py-4`}>PAYMENT GATEWAY</h2>
            <a 
              href="https://sociabuzz.com/seleneshop/tribe" 
              target="_blank" 
              className="group relative inline-flex flex-col md:flex-row items-center gap-8 bg-white/[0.03] border border-white/10 rounded-[40px] p-10 md:p-14 hover:border-purple-500/50 hover:bg-white/[0.06] transition-all duration-700"
            >
              <div className="w-20 h-20 bg-purple-500/10 rounded-3xl flex items-center justify-center text-4xl group-hover:rotate-12 group-hover:bg-purple-500/20 transition-all duration-500 shadow-inner">🛡️</div>
              <div className="text-center md:text-left">
                <span className={`${fantasyFont} block font-bold text-white text-3xl mb-2`}>SOCIABUZZ GATEWAY</span>
                <p className="text-[11px] text-white/40 uppercase tracking-[0.3em] font-bold">Verifikasi otomatis untuk QRIS & E-Wallet.</p>
              </div>
        ...
        <footer className="relative z-10 py-20 text-center border-t border-white/5 bg-black/60 backdrop-blur-md">
          <p className={`${fantasyFont} text-white/30 text-[12px] tracking-[0.8em] mb-6`}>SELENE SHOP • ESTABLISHED 2024</p>
